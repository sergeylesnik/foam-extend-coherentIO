...
#   include "createPolyMesh.H"
    ...
    if (runTime.writeFormat() == IOstream::COHERENT)
    {
...
/*
    if (runTime.writeFormat() == IOstream::COHERENT)
    {
        ...
        string pathname = mesh.pointsInstance()/mesh.meshDir();
        ...
        {
            Info << "Writing partitionStarts" << endl;
            bool reduce = true;
            Offsets partitionStarts(mesh.nCells(), reduce);
            labelList partitionEnd
            (
                1,
                partitionStarts[Pstream::myProcNo()].second
            );
